...
todos:
  - id: backend-sub-cache
    content: 订阅 /host_sw_version，缓存 version + recv_ms，注入 /api/overview
    status: completed
  - id: frontend-display
    content: Home System 面板顶部显示主机软件版本 + i18n
    status: completed
  - id: build-verify
    content: 编译 sysmonit 并验证 overview 回显与超时回退
    status: completed
isProject: false
---
